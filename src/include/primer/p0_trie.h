...
class TrieNode {
 public:
  ...
  TrieNode(TrieNode &&other_trie_node) noexcept {
    ...
    // unique_ptr需要使用move
    this->children_ = std::move(other_trie_node.children_); // move 左值转右值
  }
   ...
   */
  std::unique_ptr<TrieNode> *InsertChildNode(char key_char, std::unique_ptr<TrieNode> &&child) { // 右值引用
    if (this->children_.find(key_char) != this->children_.end() || key_char != child->GetKeyChar()) {
      return nullptr;
    }
...
class TrieNodeWithValue : public TrieNode {
 ...
 public:
  ...
  TrieNodeWithValue(TrieNode &&trieNode, T value) : TrieNode(std::forward<TrieNode>(trieNode)) {
    // 显式调用父类的构造函数，std::forward<type>() 和 move 的差不多，但是有区别
    // 简单来说，遇到右值就转发右值并启动移动语义，遇到左值就转发左值启动复制语义。
    this->value_ = value;
    this->is_end_ = true;
  }
...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    for (int i = 0; i < len; i++) {
      if (cur->get()->GetChildNode(key[i]) == nullptr) {
        cur->get()->InsertChildNode(key[i], std::make_unique<TrieNode>(key[i]));
      }
      cur = cur->get()->GetChildNode(key[i]);
    }
    ...
    *cur = std::make_unique<TrieNodeWithValue<T>>(std::move(*(cur->get())), value);
    latch_.WUnlock();
    return true;
  }
   ...
   */
  void RemoveInner(const std::string &key, size_t i, std::unique_ptr<TrieNode> *cur, std::unique_ptr<TrieNode> *prev,
                   bool *success) {
    // 返回值表示传入节点是否可以删除，可以删除的条件是该节点无子节点且非尾节点。函数内判断当前节点的子节点是否可以删除，
    // 并返回当前节点是否可以删除。出口是递归到了传入key的尾节点，取消尾节点标记，并返回是否可以删除。该函数调用前还需要判断一下key是否存在。
    if (i == key.size()) {
      *success = true;
      cur->get()->SetEndNode(false);
      if (!cur->get()->HasChildren() && cur->get()->IsEndNode()) {
        prev->get()->RemoveChildNode(key[i - 1]);
      }
      return;
    }
    if (!cur->get()->HasChild(key[i])) {
      return;
    }

    RemoveInner(key, i + 1, cur->get()->GetChildNode(key[i]), cur, success);
    if (!cur->get()->HasChildren() && !cur->get()->IsEndNode()) {
      prev->get()->RemoveChildNode(key[i - 1]);
    }
  }
  ...
  bool Remove(const std::string &key) {
    ...
    bool success = false;
    latch_.WLock();
    RemoveInner(key, 0, &root_, nullptr, &success);
    latch_.WUnlock();
    return success;
  }
